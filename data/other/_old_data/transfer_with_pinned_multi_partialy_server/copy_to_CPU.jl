...
include("create_pinned_array_if_posible.jl")

function copy_to_CPU(CPU_array, GPU_array, backend, type, chunk_size, config, times_array = [[], []])
    ...
    push!(times_array[1], time_allocation)
    ...
    # @show time_transfer
    push!(times_array[2], time_transfer)
