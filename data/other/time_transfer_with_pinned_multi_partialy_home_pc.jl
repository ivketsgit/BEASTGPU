...
for e in system_matrix_size
    ...
    open("data/other/transfer_with_pinned_multi_partialy_home_pc/$(e)_full.txt", "a") do file
        println(file, """
                Manual Benchmark of GPU-to-CPU transfer duration $(total_duration) seconds over $(num_runs) runs:
                $(times)
                """)
    end
    open("data/other/transfer_with_pinned_multi_partialy_home_pc/$(e)_allocation.txt", "a") do file
        println(file, """
                Manual Benchmark of GPU-to-CPU transfer duration $(total_duration) seconds over $(num_runs) runs:
                $(time_subparts[1])
                """)
    end
    open("data/other/transfer_with_pinned_multi_partialy_home_pc/$(e)_transfer.txt", "a") do file
        println(file, """
                Manual Benchmark of GPU-to-CPU transfer duration $(total_duration) seconds over $(num_runs) runs:
                $(time_subparts[2])
                """)
