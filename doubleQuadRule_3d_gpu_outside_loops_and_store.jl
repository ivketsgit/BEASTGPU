...
function doubleQuadRule_generic_3d_gpu_outside_loop!(result,
    ...
    womps_cart, wimps_cart, 
    x_offset, y_offset,
    instance::doubleQuadRuleGpuStrategy, writeBackStrategy::GpuWriteBack, time_table, index, ndrange_,
    should_calc=0,
...
@inline function calc_j_αG(α, womps_weight, wimps_weight, R, γ, T::doubleQuadRuleGpuStrategyOptimistic, should_calc, K, L)
    return α * womps_weight * wimps_weight * i4pi / max(R, 1e-10) * exp(-R*γ)
end
@inline function calc_j_αG(α, womps_weight, wimps_weight, R, γ, T::doubleQuadRuleGpuStrategyRepaire, should_calc, K, L)
    return - α * womps_weight * wimps_weight * i4pi / max(R, 1e-10) * exp(-R*γ)
end
@inline function calc_j_αG(α, womps_weight, wimps_weight, R, γ, T::doubleQuadRuleGpuStrategyShouldCalculate, should_calc, K, L)
    K_offset = K - 1
...
# function calc_j_αG(α, womps_weight, wimps_weight, R, γ, T)
#     return α * womps_weight * wimps_weight * i4pi / max(R, 1e-10) * exp(-R*γ)
# end
